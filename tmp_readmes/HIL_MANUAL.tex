...
\subsection{Verifying the working copy}

\begin{lstlisting}[style=sh]
git log --oneline -1                 # expect a tmp_readmes commit at the tip
ls tmp_readmes/
# HIL_MANUAL.tex  IMPROVEMENTS.md  OPERATING_MANUAL.md
# hg_dagger_architecture.tex  hg_dagger_architecture.pdf
\end{lstlisting}

The \texttt{ai\_worker-wip} branch also contains a \texttt{tmp\_readmes}
directory, but with a different and older set of files. The presence of the
directory is therefore not sufficient evidence of the correct branch; the file
list is.

\subsection{Documentation directory is excluded from version control}

\caution{\texttt{tmp\_readmes/} appears in \texttt{.gitignore}. The files listed
above are tracked because they were added with \texttt{git add -f}; a file newly
created in that directory will not appear in \texttt{git status} and will not be
committed by \texttt{git add .} or \texttt{git commit -a}. New documentation
must be staged explicitly:}

\begin{lstlisting}[style=sh]
git add -f tmp_readmes/<new file>
\end{lstlisting}

\subsection{Building this manual}

The source is \texttt{tmp\_readmes/HIL\_MANUAL.tex}. It requires no packages
beyond a standard TeX Live installation and compiles unmodified on Overleaf.
Two passes are required for cross-references.

\begin{lstlisting}[style=sh]
pdflatex HIL_MANUAL.tex
pdflatex HIL_MANUAL.tex
\end{lstlisting}

Figure placeholders are listed in Appendix~C. Each is a dashed frame naming the
file it expects; replace the \texttt{\textbackslash placeholder} call with
\texttt{\textbackslash includegraphics} once the image exists.
...
\midrule
\texttt{fig/session\_overview.png}      & Work cell during a session\\
\texttt{fig/ui\_online\_rl.png}         & Operator interface during an intervention\\
