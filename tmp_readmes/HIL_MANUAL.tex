...
\includegraphics[width=\linewidth]{figs/camera_views.png}
\caption{The four recorded streams, one frame from each. The wrist cameras are
mounted such that the upper half of the frame is ceiling.}
\end{figure}
...
\caption{The head camera under each method. Letterboxing preserves the field of
view at the cost of padding; a centred square crop removes the outer 44 per cent
of the width, which is where both arms operate.}
\end{figure}
...
\midrule
Head  & Static context view. \textbf{Approximately half of its motion energy}, and 59 per cent during the driver grasp, lies in the lateral bands a centred square crop would remove; 47.8 per cent measured on the episode shown in the figure. The smallest square retaining 90 per cent of that motion still costs 36 per cent padding.\\
...
\begin{figure}[h]\centering
\includegraphics[width=\linewidth]{figs/crop_evidence.png}
\caption{Left: column-wise motion energy over one episode of the head camera.
The two maxima correspond to the working volumes of the two arms and fall in the
bands a centred square crop would remove; the minimum is the static table
surface. Right: a wrist frame, with the region removed by the bottom-anchored
crop shaded.}
\end{figure}
...
\caption{The three inputs as presented to the model. The head retains its full
field of view behind horizontal padding; the wrist views are cropped to the
lower square, which removes the ceiling.}
\end{figure}
