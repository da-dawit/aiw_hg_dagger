...
{\large FFW SG2 follower \quad A2 leader \quad GR00T-N1.7 policy}\\[24mm]
...
\textsc{Audience.} Operators of the robot, and engineers modifying the recording
or conversion pipeline.

\textsc{Basis.} Verified against \texttt{cyclo\_intelligence\_backup@main} and
\texttt{aiw\_backup@feature-a2}. Quantities are measured unless stated
otherwise; unverified claims are identified as such.
\end{minipage}
...
lift and mobile base are never commanded by the policy.

\subsection{Shared command topic}

The teleoperation node and the policy publish to the same arm-command topic.
There is no separate policy channel. Two consequences follow.
...
\placeholder{fig/session_overview.png}{Work cell during a session: follower arms
engaged on the task, leader arms held by the operator, operator interface
visible.}{5cm}
...
\midrule
\texttt{da-dawit/aiw\_backup}                & \texttt{feature-a2} & Robot-side packages; this manual\\
...
The first two are private; access is by SSH key.
