...
The training process never reads an intervention column. Neither
\texttt{dagger\_human} nor \texttt{dagger\_auto} contains one, and Isaac-GR00T
makes no reference to \texttt{intervention}, \texttt{task\_is\_policy} or
\texttt{sample\_weight} anywhere in its source.

The label is a build-time artefact. It is read once, at export, to determine
which output dataset each frame belongs to, and is then discarded.

\begin{center}
\small
\begin{tabular}{@{}rl@{}}
\toprule
\textbf{Stage} & \textbf{Use of the label}\\
\midrule
Conversion & Computed from the inference phase\\
Export     & Read to partition frames; then dropped\\
Training   & Not present. The partition carries the distinction\\
\bottomrule
\end{tabular}
\end{center}

This follows from Isaac-GR00T having no per-frame loss weighting: a column would
be of no use to the trainer even if it were present. The corrections are
weighted by assigning whole datasets a mixture ratio instead.

\remark{The consequence is that a missing column obstructs the \emph{partition},
not the training. The labels are required only long enough to divide the frames,
which is why a reconstruction adequate for that purpose is sufficient, and why
the reference session could be trained at all despite the export lacking them.}
